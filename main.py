...
while choice.lower() != 'q':
    ...
    if choice == "3":
        ...
        if level == 1:
            while (
                new_game.list_gamers[0].get_score() < new_game.get_score_max()
                and new_game.list_gamers[1].get_score() < new_game.get_score_max()
            ):
                i = 0
                card_available = ch.create_cards_list()
                if i % 2 == 0: 
                    new_game, card_available = ch.get_first_chance_card(new_game, card_available)
                while len(card_available) != 0:
                    ...
                    # humain play 
                        ...
                        etat_combinition = ch.verify_combination(your_card, list_card_you_want_to_get_it)
